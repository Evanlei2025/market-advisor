...
- **要闻纪律：news 与 product_news 中提到的每一条新闻，都必须是上文"财联社当日要闻"列表中出现的（可概括原话），不得提及输入中不存在的任何新闻事件或板块行情；product_news 必须完成"新闻→影响→客户持有产品"的映射，逐一说明对哪个持有产品有影响；若列表中没有与持有产品相关的新闻，写"今日无直接相关要闻"**
